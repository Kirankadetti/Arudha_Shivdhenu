...
function buildNav() {
  ...
  const logoPath = isRoot ? 'assets/tej_pratibimb_peeth_emblem.png' : '../assets/tej_pratibimb_peeth_emblem.png';
...
function buildFooter() {
  ...
  <footer>
    <div class="wrap">
      ...
      <div class="ft-bottom">
        <p>© 2024 Shivdhenu. All rights reserved. Made with 🙏 and devotion.</p>
...
function prodCard(p) {
  ...
  <div class="prod-card reveal" onclick="openProductModal(${p.id})" role="button" tabindex="0" aria-label="View details for ${p.name}" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openProductModal(${p.id})}">
    ...
    <div class="prod-body">
      ...
      ${p.status === 'available'
        ? `<a href="https://wa.me/${waNum}?text=${encodeURIComponent(waMsg)}" target="_blank" rel="noopener" class="prod-wa" onclick="event.stopPropagation()"><span class="wa-svg-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M21 12a8 8 0 0 1-8 8H5l2-4a8 8 0 1 1 14-4Z"/>
            </svg>
            </span> Order on WhatsApp</a>`
        : `<div class="prod-soon-label"><span class="c-ico" style="width:16px;height:16px;border:none">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="pm-box" role="dialog" aria-modal="true" aria-labelledby="pm-title">
      ...
      <div class="pm-grid">
        <div class="pm-gallery">
          <div class="pm-main-img"><img id="pm-main-img" src="" alt=""></div>
...
function openProductModal(id) {
  ...
  document.getElementById('pm-main-img').src = images[0];
  document.getElementById('pm-main-img').alt = p.name;

  document.getElementById('pm-thumbs').innerHTML = images.map((src, i) => `
    <div class="pm-thumb ${i===0?'active':''}" onclick="setProductModalImage('${src.replace(/'/g,"\\'")}', this)">
      <img src="${src}" alt="${p.name} thumbnail ${i+1}">
    </div>
  `).join('');
  /* Hide thumbnail strip entirely when there's only one image to avoid empty clutter */
  document.getElementById('pm-thumbs').style.display = images.length > 1 ? 'flex' : 'none';
...
function setProductModalImage(src, thumbEl) {
